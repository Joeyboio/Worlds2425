...
void discard(){
  while (!IntLim.get_value()) {
    Intake.move(-120);
  }
  Intake.move_relative(10, 120);
  Intake.move(0);
  pros::delay(250);
}
...
void colorsort(bool TeamColor){
  Optical.set_led_pwm(100);
  int hue = Optical.get_hue();

  bool isRed = (hue >= 345 || hue <= 15);
  bool isBlue = (hue >= 210 && hue <= 270);
...
void initialize() {
  ...
  ez::as::auton_selector.autons_add({
      {"Prog Skills", programmingSkills},
      {"RedPositive", RedPositiveSafe},
      {"BluePositiveSafe", BluePositiveSafe},
      {"RedNegative", RedNegative},
      {"BlueNegative", BlueNegative},
      {"Prog Skills", programmingSkills},
      {"RedNegative", RedNegative},
      {"RedPositive", RedPositiveSafe},
      {"BlueNegative", BlueNegative},
